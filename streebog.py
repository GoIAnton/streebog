...
def L(in_data):
    old_str_data = [0] * 64
    for i in range(64):
        old_str_data[i] = '{:08b}'.format(in_data[i])
    str_data = ''.join(old_str_data)
    old_data = [int(num) for num in str_data]
    new_data = [0] * 8
    new_str_data = [0] * 8
    for i in range(8):
        k = i * 64
        for j in range(64):
            if old_data[k + j] == 1:
                new_data[i] ^= A[j]
        new_str_data[i] = '{:016x}'.format(new_data[i])
    str_data = ''.join(new_str_data)
    out_data = [int(str_data[i:i+2], 16) for i in range(0, 128, 2)]
    return out_data


def LPS(in_data):
    return L(P(S(in_data)))


def create_iter_K(K1):
    K[0] = K1
    for i in range(1, 13):
        K[i] = LPS(X(K[i-1], C[i-1]))
...
def to_int(num):
    return int(''.join(['{:02x}'.format(n) for n in num]), 16)


def to_arr(arr):
    return [int('{:0128x}'.format(arr)[i:i+2], 16) for i in range(0, 128, 2)]
...
def H(M):
    ...
    while len(M) >= 128:
        str_m = M[-128:]
        m = [int(str_m[i:i+2], 16) for i in range(0, 128, 2)]
        h = g(N, h, m)
        N = to_arr((to_int(N) + 512) % 2**512)
        int_Sig = (int_Sig + int(str_m, 16)) % 2**512
        M = M[:-128]
    m_len = len(M) * 4
    M = int(M, 16)
    M = '{:0128x}'.format(M)
    ...
    for i in range(128):
        if m[i] != '0':
            ...
            break
    M = ''.join(m)
    m = [int(M[i:i+2], 16) for i in range(0, 128, 2)]
    h = g(N, h, m)
    print(to_int(N) + 512 % 2**512)
    N = to_arr(to_int(N) + m_len % 2**512)
    int_Sig = (int_Sig + int(M, 16)) % 2**512
    Sig = to_arr(int_Sig)
    h = g([0] * 64, h, N)
    h = g([0] * 64, h, Sig)
    return h
...
if __name__ == '__main__':
    K1 = [
        179, 131, 252, 46, 206, 212, 165, 116,
        179, 131, 252, 46, 206, 212, 165, 116,
        179, 131, 252, 46, 206, 212, 165, 116,
        179, 131, 252, 46, 206, 212, 165, 116,
        179, 131, 252, 46, 206, 212, 165, 116,
